...
from flag_chat_case import ALLOWED_CATEGORIES, ALLOWED_PRIORITIES, ALLOWED_SOURCES, create_case
...
async function search(){status.textContent='Loading…';list.replaceChildren();try{const response=await fetch(`/api/conversations?q=${encodeURIComponent(q.value)}&offset=${offset}&limit=${pageSize}`);if(!response.ok)throw new Error(`HTTP ${response.status}`);const data=await response.json();const first=data.total?offset+1:0,last=Math.min(offset+data.items.length,data.total);status.textContent=`Showing ${first}–${last} of ${data.total}`;pageLabel.textContent=`Page ${Math.floor(offset/pageSize)+1}`;previous.disabled=offset===0;next.disabled=offset+pageSize>=data.total;for(const chat of data.items){const b=element('button','item');b.type='button';b.append(element('div','preview',chat.preview||'No user-text preview available'),element('div','meta',`${chat.traceCount||0} turns · ${chat.firstTimestamp||'no timestamp'}`),element('div','id',chat.chatId));b.onclick=()=>openChat(chat.chatId);list.append(b)}}catch(error){status.textContent=`Unable to load conversations: ${error.message}`;pageLabel.textContent='';previous.disabled=true;next.disabled=true}}
...
def handler_factory(source: Path, index_path: Path, flag_output: Path):
    class Handler(BaseHTTPRequestHandler):
        ...
        def open_flag_ids(self) -> set[str]:
            queue = flag_output / "flagged-cases.jsonl"
            if not queue.exists():
                return set()
            return {json.loads(line)["chatId"] for line in queue.read_text(encoding="utf-8").splitlines()
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if request.path == "/api/conversations":
                ...
                self.send_json({"total": total, "items": [{"chatId": row["chat_id"], "firstTimestamp": row["first_timestamp"], "lastTimestamp": row["last_timestamp"], "traceCount": row["trace_count"], "preview": row["preview"], "flagged": row["chat_id"] in open_ids} for row in rows]})
                return
            if request.path.startswith("/api/conversations/"):
                ...
                if row is None:
                    ...
                    self.send_json(load_conversation(source, row["byte_offset"], row["byte_length"]))
                return
